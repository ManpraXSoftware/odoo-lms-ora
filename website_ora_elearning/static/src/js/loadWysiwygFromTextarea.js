...
import { attachComponent } from "./owl_utils";
...
export async function loadWysiwygFromTextarea(parent, textarea, options) {
    ...
    if (!currentOptions.value.trim()) {
        currentOptions.value = '<p o-we-hint-text="Type \"/\" for commands" class="o-we-hint">[]<br></p>';
    }

    await ensureJQuery();
    await loadBundle("web_editor.assets_wysiwyg");
    const { Wysiwyg } = await odoo.loader.modules.get('@html_editor/wysiwyg');
    let wysiwyg;
